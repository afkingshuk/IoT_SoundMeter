...
#define DMA_DEVICE_ID "22070001"

char willMsg[50] = "GPRS 22070001 disconnected!";
uint8_t imei[16] = "";
uint8_t sPayload[64] = "";
...
void OnTimerPublish(void* param)
{
    ...
    getSoundLevel();
    snprintf(sPayload, sizeof(sPayload), "%s,%d ", DMA_DEVICE_ID, soundlevelvalue);
    // snprintf(count, sizeof(count), "Counter %d", counter);

    Trace(1,"MQTT OnTimerPublish");
    err = MQTT_Publish(client,PUBLISH_TOPIC,sPayload,strlen(sPayload),1,2,0,OnPublish,NULL);
    if(err != MQTT_ERROR_NONE)
        Trace(1,"MQTT publish error, error code:%d",err);
